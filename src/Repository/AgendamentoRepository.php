...
class AgendamentoRepository extends ServiceEntityRepository
{
    ...
    public function listaAgendamentoPorId($baseId, $id)
    {
        $sql = "SELECT a.id, data, concluido, pronto, horaChegada, metodo_pagamento, horaSaida, horaChegada, 
                ...
                LEFT JOIN homepet_{$baseId}.agendamento_pet_servico aps ON (a.id = aps.agendamentoId AND aps.estabelecimento_id='{$baseId}')
    ...
    public function listaApsPorId($baseId, $idAgendamento)
    {
        $sql = "SELECT aps.id, agendamentoId, petId, servicoId, p.nome AS pet_nome, c.nome AS cliente_nome, s.nome AS servico_nome, s.valor
            ...
            JOIN homepet_{$baseId}.pet p ON (p.id = aps.petId AND p.estabelecimento_id='{$baseId}')
            JOIN homepet_{$baseId}.cliente c ON (c.id = p.dono_id AND c.estabelecimento_id='{$baseId}')
            JOIN homepet_{$baseId}.servico s ON (s.id = aps.servicoId AND s.estabelecimento_id='{$baseId}')
    ...
    {
        $sql = "INSERT INTO homepet_{$baseId}.agendamento_pet_servico (estabelecimento_id, agendamentoId, petId, servicoId)
    ...
    {
        $sql = "DELETE FROM homepet_{$baseId}.agendamento 
            WHERE estabelecimento_id = '{$baseId}' AND id = :id";
