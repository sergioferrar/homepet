...
class PetRepository extends ServiceEntityRepository
{
    ...
    {
        $sql = "INSERT INTO homepet_{$baseId}.pet 
    ...
    {
        $sql = "DELETE FROM homepet_{$baseId}.pet WHERE estabelecimento_id = '{$baseId}' AND id = :id";
    ...
    {
        $sql = "SELECT COUNT(*) FROM homepet_{$baseId}.pet WHERE estabelecimento_id = :baseId";
    ...
    public function listarVacinasPendentes($baseId): array
    {
        $sql = "SELECT v.id, v.pet_id, v.tipo, v.data_aplicacao, v.data_validade,
                ...
                JOIN homepet_{$baseId}.pet p ON v.pet_id = p.id
                JOIN homepet_{$baseId}.cliente c ON p.dono_id = c.id
    ...
    public function listarPetsInternados($baseId): array
    {
        $sql = "SELECT p.id, p.nome, p.raca, p.especie, c.nome as tutor
                FROM homepet_{$baseId}.pet p
                JOIN homepet_{$baseId}.cliente c ON p.cliente_id = c.id
                WHERE p.internado = 1
                  AND p.estabelecimento_id = :baseId
                ORDER BY p.nome";
