...
class PagSeguroService{
    ...
    private $url_retorno   = "http://SEUSITE/pagseguro/retorno";
    ...
    */
    ...
    public function __construct(HttpClientInterface $client, string $email, string $token){
        $this->email_token = "?email=".$email."&token=".$token;
        $this->url .= $this->email_token;
        $this->email = $email;
        $this->token_oficial = $token;
    }

    private function generateUrl($dados, $produto, $retorno){
        //Configurações
        ...
        //Itens
        $data['itemId1'] = $produto["id"];//'0001';
        $data['itemDescription1'] = $produto['titulo'];
        $data['itemAmount1'] = number_format($produto['valor'],2,".","");
        $data['itemQuantity1'] = '1';
        $data['itemWeight1'] = '0';
        
        //Dados do pedido
        $data['reference'] = '001F002';      
        ...
        $data['redirectURL'] = 'http://localhost'.$retorno;
        // dd($dados, $produto, $retorno, $data);

        return http_build_query($data);
    }
    
    public function executeCheckout($dados, $produto, $retorno){
        
        // if($dados['codigo_pagseguro']!="" && $dados['codigo_pagseguro']!=null){
        //     header('Location: '.$this->url_redirect.$dados['codigo_pagseguro']);
        // }
        
        $dados = $this->generateUrl($dados, $produto, $retorno);
    ...
    public function gerarUrlPagamento(string $code)
    {
        return $this->url_redirect . $code;
    }
}